...
// Возврат выбора после перебора альтернатив и переключения Доктором: VPN остаётся включённым, если
// он был включён на старте Доктора или поднят сейчас. Правило пикера «туннель не поднят → только
// цель, намерение OFF» здесь неверно: туннель лежит, потому что его только что остановил сам Доктор
// (разбор журнала 26.09: полный Доктор оставлял VPN выключенным после неудачной последней
// альтернативы — «сам выключается»).
inline bool restoreKeepsConnected(bool connectedAtStart, bool tunnelUpNow)
{
    return connectedAtStart || tunnelUpNow;
}
