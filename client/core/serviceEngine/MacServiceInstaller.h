#pragma once
// AVPN (macOS desktop): авто-установка root-демона Tribe-service из ВШИТОГО в app
// подписанного/опечатанного runtime payload — без терминала, через системный промпт.
...
// Установленный демон УСТАРЕЛ относительно вшитого в app? Сверяет маркер версии (SHA-256
// закрытого manifest: daemon + AWG/OpenVPN/tun2socks + Xray data + Frameworks + PF):
...
enum class MacServiceInstallReason {
    FirstInstall,
    Update,
    Repair,
};
...
//
// Фаза 1 — ТОЛЬКО главный поток: пояснительный диалог с точным действием (установка, обновление
// или восстановление), причиной и предупреждением о следующем системном запросе пароля.
...
// Explicit shipped cleanup route.  It uses the same root-owned signed-app
// snapshot as installation, removes only the exact Tribe service namespace,
// and deliberately leaves the GUI bundle and account data untouched.
