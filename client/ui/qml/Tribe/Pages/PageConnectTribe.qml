...
PageType {
        ...
        Rectangle {
            ...
            MouseArea {
                ...
                onClicked: {
                    ...
                    // долгоживущий токен не светится в истории браузера/логах. Устройство вшито в сам
                    // wl-токен (дни зачислятся на ЭТО устройство). Ошибка/нет движка → голый кабинет. // AVPN
    ...
    // (попытка подключения): хост здесь, а не в PageStart (реш. владельца 2026-07-12 —
    // на уведомлениях/других вкладках не показывать). Вкладки существуют по одной
    // (clear+replace) — при уходе с главной попап уничтожается вместе со страницей
    // (депт-контракт «назад» чинится в Component.onDestruction шита). z:150 — выше
    // контента страницы (max z:40), ниже force-update гейта (z:9999).
    TribeWhitelistSheet {
        z: 150
        anchors.fill: parent
    }
