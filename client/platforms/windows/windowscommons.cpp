...
constexpr const char* VPN_NAME = "TribeVPN"; // AVPN: имя туннеля/конфига — не делить с офиц. Amnezia
...
int WindowsCommons::VPNAdapterIndex() {
  ...
  for (const auto& adapter : adapterList) {
    if (adapter.humanReadableName().contains("TribeVPN")) { // AVPN: ищем СВОЙ адаптер, не офиц. Amnezia
