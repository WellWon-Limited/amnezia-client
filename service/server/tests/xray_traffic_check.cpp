...
int main()
{
    ...
    // 5. Откат ТОЛЬКО ОДНОГО счётчика (ревью волны, MINOR-6): откатившийся считается с нуля,
    //    второй — обычной дельтой. Прибавлять абсолютное значение «выжившего» нельзя: это скачок
    //    размером с аптайм интерфейса, который маскирует «rx стоит» (DEAD-критерий xray, §22.5).
    {
        XrayTrafficAccumulator acc;
        acc.sample(1000, 1000);
        acc.sample(10, 1200); // rx откатился (→ +10), tx вырос обычной дельтой (→ +200)
        CHECK(acc.totalRx == 10);
        CHECK(acc.totalTx == 200);
        CHECK(acc.resets == 1);
    }

    // 5a. Зеркальный случай: откатился tx, rx растёт — ложного скачка rx быть не должно
    //     (иначе мёртвый data-plane выглядит живым).
    {
        XrayTrafficAccumulator acc;
        acc.sample(1'000'000, 1'000'000);
        acc.sample(1'000'000, 5); // rx СТОИТ (+0), tx откатился (+5)
        CHECK(acc.totalRx == 0);
        CHECK(acc.totalTx == 5);
        CHECK(acc.resets == 1);
        acc.sample(1'000'000, 105); // rx по-прежнему стоит — «rx стоит, tx растёт» видно честно
        CHECK(acc.totalRx == 0);
        CHECK(acc.totalTx == 105);
    }
